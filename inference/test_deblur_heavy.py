...
class Inference:
    def __init__(self, args):
        ...
        self.data_list = json.load(
            open(
                "/data/projects/derain/cu_video_derain/for_comparison/2021/Enhanced-Spatio-Temporal-Interaction-Learning-for-Video-Deraining/heavy.json"
            )
        )

        now_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        self.logger = Traverse_Logger(
            self.result_path, "inference_log_{}.txt".format(now_time)
        )
        ...
        with torch.no_grad():
            ...
            for v_index, v in enumerate(videos):
                ...
                for kk in range(k_len):
                    ...
                    new_h, new_w = (
                        h - h % self.size_must_mode,
                        w - w % self.size_must_mode,
                    )
                    ...
                    for ele in range(one_len):
                        output_img = (
                            output[ele].clamp(0, 1.0).permute(1, 2, 0).cpu().numpy()
                        )
                        ...
                        if self.save_image:
                            ...
                            cv2.imwrite(
                                os.path.join(self.result_path, v, "%03d.png" % index),
                                output_img[..., ::-1],
                            )
                            index = index + 1
    ...
    def numpy2tensor(self, input_seq, rgb_range=1.0):
        tensor_list = []
    ...
    def tensor2numpy(self, tensor, rgb_range=1.0):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_image", action="store_true", default=False, help="save image if true"
    )
    parser.add_argument(
        "--border", action="store_true", help="restore border images of video if true"
    )
