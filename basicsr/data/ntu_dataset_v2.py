...
class NTU_dataset(Dataset):
    ...
    def __init__(self,
        ...
        self.streak_dir = '/home/desc/projects/derain/cu_rain_video_dataset/train/rain_streak_filtered'
        self.streak_dir_list = os.listdir(self.streak_dir)

    def filter_out_garbage(self, in_list):
        return list(filter(lambda x: '._' not in x and '.DS_' not in x, in_list))

    def screen(self, imgs_in, imgs_layer, opacity):
        """
    ...
    def sort_list(self, video_frame_list):
        return sorted(
            video_frame_list,
    ...
    def __getitem__(self, idx):
        ...
        new_ip_frames = []  # frame_with_rain

        if not self.is_testing and 'motion_100' in ip_data[0]:
            alpha = random.uniform(a=0.5, b=0.9)
            rotation_angle = random.randint(a=-15, b=15)
            selected_streak_dir = os.path.join(
                self.streak_dir, random.choice(self.streak_dir_list))
            frame_list = os.listdir(selected_streak_dir)
            frame_list = self.filter_out_garbage(frame_list)
            frame_list = self.sort_list(frame_list)
            frame_list = list(map(lambda x: os.path.join(
                selected_streak_dir, x), frame_list))
            frame_count = len(frame_list)
            selected_rain_idx = random.randint(a=2, b=frame_count - 3)
            selected_frames = frame_list[selected_rain_idx -
                                         2: selected_rain_idx + 3]
            for selected_frame_path, ip_frame in zip(selected_frames, ip_frames):
                new_ip = Image.fromarray(
                    self.screen(
                        imgs_in=np.array(ip_frame).astype('float32'),
                        imgs_layer=np.array(Image.open(selected_frame_path).rotate(
                            rotation_angle)).astype('float32'),
                        opacity=alpha
                    ).astype(np.uint8)
                )
                new_ip_frames.append(new_ip)
            ip_frames = new_ip_frames

        ip_tensors, gt_tensors = [], []

        # the whole clip should apply same transform.
        transform_params = self.get_transform_params(
            ip_frames[0].size[0], ip_frames[0].size[1])
        for ip_frame, gt_frame in zip(ip_frames, gt_frames):
            ip_frame, gt_frame = self.apply_transform(
                ip_frame, gt_frame, transform_params)
        ...
        assert ip_tensors.shape == gt_tensors.shape
    ...
    def apply_transform(self, ip_frame, gt_frame, params):
        x0, y0, hflip_rnd, vflip_rnd, deg = params['x0'], params[
            'y0'], params['hflip_rnd'], params['vflip_rnd'], params['degree']
...
def get_dataloader(dataset, batch_size=2, shuffle=True, num_workers=8):
