...
class NTU_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ip_tensors = torch.stack(ip_tensors, 0).permute(0, 2, 3, 1)
        gt_tensors = torch.stack(gt_tensors, 0).permute(0, 2, 3, 1)
        # tensors shape:  temporal window size (T) x H x W x Channel (C)
        return (
            ip_tensors,
            gt_tensors,
