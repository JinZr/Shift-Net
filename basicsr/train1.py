    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if dist.get_rank() == 0 and current_iter % opt["logger"]["print_freq"] == 0:
                ...
                print(
                    "Loss at iteration", current_iter, model.get_current_log()["l_pix"]
                )
                msg_logger(log_vars)
            ...
            if (
                dist.get_rank() == 0
                and current_iter % opt["logger"]["save_checkpoint_freq"] == 0
            ):
            ...
            if dist.get_rank() == 0 and current_iter % 2000 == 1:
                ...
                cv2.imwrite(
                    os.path.join(
                        opt["path"]["visualization"], "input_%d.png" % (current_iter)
                    ),
                    input_s[:, :, ::-1] * 255,
                )
                cv2.imwrite(
                    os.path.join(
                        opt["path"]["visualization"], "gt_%d.png" % (current_iter)
                    ),
                    gt_s[:, :, ::-1] * 255,
                )
                cv2.imwrite(
                    os.path.join(
                        opt["path"]["visualization"], "out_%d.png" % (current_iter)
                    ),
                    out_s[:, :, ::-1] * 255,
                )
            # exit(0)
